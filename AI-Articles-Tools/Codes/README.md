...
│   ├── Configs/                     # 全局配置：config.toml（含密钥，gitignore）+ config.example.toml（模板）
│   └── Database/                    # 全局 SQLite 数据库（ai_articles.db，各分类共享，gitignore）
└── AI-Articles-Tools/
    ├── Codes/                       # 本模块全部代码
    │   ├── requirements.txt
    │   ├── src/
    │   │   ├── cli.py          # 命令行入口（collect / pipeline / create / list）
    │   │   ├── config.py       # 配置加载（默认读 AI-Global/Configs，环境变量 > config.toml）
...
> 配置与数据库已全局化：所有分类共用 `AI-Global/Configs/config.toml` 与
> `AI-Global/Database/ai_articles.db`；各分类的 `Articles/`、`OutArticles/` 仍留在本分类目录。
...
cp AI-WeMedia-Tools/AI-Global/Configs/config.example.toml AI-WeMedia-Tools/AI-Global/Configs/config.toml
# 编辑 config.toml 填入 LLM / 图片 / ASR 的 base_url、api_key、model
# （全局配置被 .gitignore 忽略，不会误提交密钥；其他分类可共用同一份）
...
> 配置默认读取 `AI-Global/Configs/config.toml`（全局共享），也可用 `--config <path>` 指定。
> 数据库文件位于 `AI-Global/Database/ai_articles.db`（全局共享，由 `paths.db` 配置），自动创建。
...
- **ASR**：视频采集在 `mock` 模式返回占位文案；真实模式走 yt-dlp 下载 + ffmpeg
  抽音频 + ASR 转写。
- **路径**：采集/二创产物默认落在 `AI-Articles-Tools/Articles` 与 `AI-Articles-Tools/OutArticles`，
  由配置 `paths.articles_dir` / `paths.out_dir`（相对本分类目录解析）决定；
  数据库由 `paths.db` 解析到全局目录 `AI-Global/Database`（相对项目根目录解析），各分类共享。
